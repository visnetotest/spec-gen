...
> and `openspec/specs/architecture/spec.md` (AdditiveBitemporalMemorySchema). 12 e2e tests
> (`bitemporal-memory.test.ts`) over a real git repo + an orient contradiction case; full suite green
> (3,906 passed); end-to-end dogfood confirmed against a real `analyze` run. See "Implementation
...
**Verification:** `bitemporal-memory.test.ts` (12 e2e cases over a real git repo: validFromCommit
reproducibility, supersede→asOf history, unreconciled + clear, typed filter, changedSince, dedup) plus
an orient contradiction case in `orient-memory-freshness.test.ts`. Full suite green (3,906 passed, 2
