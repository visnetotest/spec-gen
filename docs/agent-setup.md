...
MCP clients load all tool schemas at session start. With 58 tools, this costs ~14k tokens of `tools/list` before any work begins (Spec 28 measured it; the lossless server-side trim is only ~2%, so the real lever is deferral). Claude Code supports `alwaysLoad: false` (deferred, default) — tools load only when the agent searches for them via Tool Search.
...
- **`openlore`** exposes all 58 tools deferred — loaded on demand when the agent uses Tool Search (e.g. "find tool for BFS graph traversal").
