...
- **Dockerfile edges:** `FROM` → an earlier same-file stage (when the base names one) or the external base image; `COPY/ADD --from=` → a stage (by name or build index) or an external image. `FROM scratch` and `FROM ${ARG}` (dynamic) produce no edge.
