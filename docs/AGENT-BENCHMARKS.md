...
## Round 4 — lean orient (2026-06-03, Spec 27)

Spec 27 added a `lean` orient mode (navigation core only; enrichment behind `expand` handles) that
cuts the orient payload **2269 → 1357 tokens (−40%)** deterministically. Does that move the
shallow-task loss? **No.** express "who calls X" with **lean** orient: WITHOUT $0.024 / 3 turns vs
WITH-lean $0.049 / 5 turns (**+107%**, N=2) — no better than rich orient (+59%). The loss is
**structural**: the baseline already finishes a trivial famous-repo lookup in ~3 turns / ~1.9k tokens,
so *any* orient call — one extra round-trip + its response — costs more than the whole answer. Lean is
a real efficiency win on every orient call (especially deep / borderline ones), but it **cannot
remove the round-trip**, which dominates on trivial tasks. Honest conclusion: ship lean as the cheaper
