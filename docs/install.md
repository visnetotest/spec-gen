...
| `--preset <name>` | Wire the MCP server to a tool preset: `navigation` (the lean default), `minimal`, `memory`, `verify`, `federation`, `coordination`, or `full`. Omit it for the lean navigation surface; pass `--preset full` to wire all 69 tools (the prior default). |
