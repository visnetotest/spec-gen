...
**Infrastructure-as-Code**: Terraform/HCL · Kubernetes · Helm · CloudFormation · Ansible · Pulumi · AWS CDK · CDKTF · Dockerfile · Docker Compose — IaC resources and their references are projected onto the same graph as application code, so `orient`, `search_code`, `get_subgraph`, and `analyze_impact` answer "what is the blast radius of changing this security group / ConfigMap / IAM role / base image?" with zero new tooling. For containers, a compose service's `build:` resolves cross-file to the Dockerfile stage it builds, whose `FROM` resolves to a base image — so one `analyze_impact` on a base image surfaces every stage and service that would rebuild. See [docs/iac.md](docs/iac.md).
